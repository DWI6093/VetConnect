...
export class NegociosServicio {
  ...
  async obtenerCatalogo(
    ...
    try {
      const negocios = await this.negociosRepositorio.buscarPorNombre(
        nombre,
        pagina,
        limite,
      );
  ...
  async obtenerMisNegocios(idPropietario: number) {
    try {
      const negocios =
        await this.negociosRepositorio.obtenerPorPropietario(idPropietario);
      return negocios.map((negocio) => ({
        id_negocio: negocio.id_negocio,
        nombre: negocio.nombre,
        direccion: negocio.direccion,
  ...
  async crearNegocio(idPropietario: number, datos: CrearNegocioDto) {
    try {
      const negocio = await this.negociosRepositorio.crear(
        idPropietario,
        datos,
      );
      return {
      ...
      throw new InternalServerErrorException(
        'Ocurrió un error interno al crear el negocio.',
      );
    }
  }
  ...
  async actualizarNegocio(
    ...
    const negocioExistente =
      await this.negociosRepositorio.obtenerPorId(idNegocio);
    ...
    try {
      const actualizado = await this.negociosRepositorio.actualizar(
        idNegocio,
        datos,
      );
      return {
      ...
      throw new InternalServerErrorException(
        'Ocurrió un error interno al actualizar el negocio.',
      );
    }
  }
  ...
  async cambiarEstadoNegocio(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Ocurrió un error interno al cambiar el estado del negocio.',
      );
    }
  }
  private async verificarPropietarioONegar(
    idNegocio: number,
    idUsuario: number,
  ) {
    const esPropietario = await this.negociosRepositorio.esPropietario(
      idNegocio,
      idUsuario,
    );
    if (!esPropietario) {
  ...
  async actualizarHorarios(
    ...
    try {
      return await this.negociosRepositorio.reemplazarHorarios(
        idNegocio,
        horarios,
      );
    } catch (error) {
  ...
  async crearServicio(
    idNegocio: number,
    idUsuario: number,
    datos: CrearServicioDto,
  ) {
    await this.verificarPropietarioONegar(idNegocio, idUsuario);
    try {
      const servicio = await this.negociosRepositorio.crearServicio(
        idNegocio,
        datos,
      );
  ...
  async eliminarServicio(idServicio: number, idUsuario: number) {
    const servicio =
      await this.negociosRepositorio.obtenerServicioConPropietario(idServicio);
  ...
  async crearProducto(
    ...
    await this.verificarPropietarioONegar(idNegocio, idUsuario);
    try {
      const producto = await this.negociosRepositorio.crearProducto(
        idNegocio,
        datos,
      );
  ...
  async eliminarProducto(idProducto: number, idUsuario: number) {
    const producto =
      await this.negociosRepositorio.obtenerProductoConPropietario(idProducto);
  ...
  async agregarImagen(
    ...
    const totalActual =
      await this.negociosRepositorio.contarImagenes(idNegocio);
    if (totalActual >= 4) {
      throw new ForbiddenException(
        'El plan Básico permite un máximo de 4 imágenes por negocio.',
      );
    }
  ...
  async eliminarImagen(idImagen: number, idUsuario: number) {
    const imagen =
      await this.negociosRepositorio.obtenerImagenConPropietario(idImagen);
  ...
  async obtenerDetalleColaborador(idNegocio: number, idUsuario: number) {
    const negocio =
      await this.negociosRepositorio.obtenerDetalleCompleto(idNegocio);
    ...
    return {
      ...
      latitud: Number(negocio.latitud),
      longitud: Number(negocio.longitud),
      servicios: (negocio.servicios ?? []).map((s: any) => ({
        ...s,
        precio: Number(s.precio),
      })),
      productos: (negocio.productos ?? []).map((p: any) => ({
        ...p,
